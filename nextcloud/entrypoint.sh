...
mkdir -p $( dirname $(cat /etc/supervisor/conf.d/supervisord.conf | grep pidfile= | grep "\.pid" | sed s/.*pidfile=// ) )
touch $( cat /etc/supervisor/conf.d/supervisord.conf  | grep pidfile= | grep "\.pid" | sed s/.*pidfile=// )
